...
def write_edf(mne_raw, fname, picks=None, tmin=0, tmax=None, overwrite=False):
    ...
    """
    if not issubclass(type(mne_raw), BaseRaw):
        raise TypeError('Must be mne.io.Raw type')
    if not overwrite and exists(fname):
        raise OSError('File already exists. No overwrite.')
    # static settings
    if splitext(fname)[-1] == '.edf':
    ...
    f = None
    try:
        f = EdfWriter(fname, n_channels=n_channels, file_type=file_type)
        channel_info = []
        data_list = []
        for i in range(n_channels):
            ch_dict = {'label': mne_raw.ch_names[i], 'dimension': 'uV', 'sample_rate': sfreq, 'physical_min': pmin,
                       'physical_max': pmax, 'digital_min': dmin, 'digital_max': dmax, 'transducer': '',
                       'prefilter': ''}
