...
def plot_connectivity_circle_arrows(con, node_names, psi, indices=None, n_lines=None, node_angles=None,
                                    node_width=None, node_colors=None, facecolor='black', textcolor='white', node_edgecolor='black',
                                    linewidth=1.5, colormap='hot', vmin=None, vmax=None, colorbar=True, title=None,
                                    colorbar_size=0.2, colorbar_pos=(-0.3, 0.1), fontsize_title=12, fontsize_names=8,
                                    fontsize_colorbar=8, padding=6., fig=None, subplot=111, interactive=True,
                                    node_linewidth=2., show=True):
    """Visualize the connectivity and directionality as a circular graph.
    ...
    for pos, (i, j) in enumerate(zip(indices[0], indices[1])):
        ...
        style = m_patches.ArrowStyle(stylename="->", angleA=0.5, angleB=0.5)

        # Actual arrow
        if psi[i][j] >= 0:  # Directionality is positive, so go the right way.
            arrow = m_patches.FancyArrowPatch(posA=(t0, r0 - 1), posB=(t1, r1 - 1), arrowstyle=style, color=color,
                                              linewidth=linewidth, mutation_scale=10)
        else:   # Directionality is negative, go the opposite way.
            arrow = m_patches.FancyArrowPatch(posA=(t1, r1 - 1), posB=(t0, r0 - 1), arrowstyle=style, color=color,
                                              linewidth=linewidth, mutation_scale=10)
